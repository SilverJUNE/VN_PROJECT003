...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_GraphicPanels : CMD_DatabaseExtension
    {
        ...
        {
            database.AddCommand("setlayermedia", new Func<string[], IEnumerator>(SetLayerMedio));
            database.AddCommand("clearlayermedia", new Func<string[], IEnumerator>(ClearLayerMedia));
        }

        private static IEnumerator SetLayerMedio(string[] data)
        {
            ...
            if(panel == null)
            {
                Debug.LogError($"Unable to grab panel '{panelName}' because it is not a vailed panel. Please check the panel name and adjust the command.");
                yield break;
            }
        ...
        private static IEnumerator ClearLayerMedia(string[] data)
        {
            ...
            else
            {
                ...
                {
                    Debug.LogError($"Could not clear laye [{layer}] on panel '{panel.panelName}'");
                    yield break;
                }
